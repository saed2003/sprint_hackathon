...
Run standalone (newest scan by default):
  python3 view3d.py
  python3 view3d.py captures/scan_20260531_1700/merged_360.ply

Or it is launched for you from drive.py (press Y after building a cloud with T).
"""
...
def main():
    ...
    if args:
        ...
        cands = glob.glob("captures/scan_*/merged_360.ply") + glob.glob("captures/*/cloud.ply")
        if not cands:
            sys.exit("No cloud found. Pass a .ply path, e.g.\n"
                     "  python3 view3d.py captures/scan_<ts>/merged_360.ply")
