...
Usage:
  ...
  .venv/bin/python merge_clouds.py captures/A captures/B captures/C
  ...
  .venv/bin/python merge_clouds.py --angle 30 captures/A captures/B captures/C
...
def main():
    ...
    if len(folders) < 2:
        sys.exit("Need at least 2 captures. Run capture.py a few times "
    ...
    print(f"merged cloud: {len(merged.points)} points "
          f"(cleaned {before - len(merged.points)} outliers) -> merged.ply")
    ...
    plt.tight_layout(); plt.savefig("merged_views.png", dpi=110); plt.close()
    print("saved -> merged_views.png")
