...
|---|---|---|---|
| **On the robot (Raspberry Pi)** | `pyrealsense2`, `numpy`, `cv2` — **no Open3D** | [rs_capture.py](rs_capture.py), [capture.py](capture.py), [scan360.py](scan360.py), [view3d.py](view3d.py), [drive.py](drive.py), [rasbot/](setup_and_api/api/robot.py) | capture + a self-contained 360 scan/merge/view |
| **On the laptop** | `+ open3d`, `matplotlib` | [make_pointcloud.py](make_pointcloud.py), [merge_clouds.py](merge_clouds.py), [render_cloud.py](render_cloud.py) | high-quality per-shot clouds + ICP merge |
| **Either** | stdlib only | [clean_captures.py](clean_captures.py) | delete captures to start fresh |
...
The robot driver [drive.py](drive.py) is the **conductor**: it owns the keyboard, calls
...
- **Key code** ([rs_capture.py:96](rs_capture.py#L96)) — `save_to()` grabs one synced frameset
...
- **Theory — back-projection.** A depth image gives only distance `Z` per pixel `(u,v)`. To get
  ...
  ([make_pointcloud.py:83](make_pointcloud.py#L83)):
  ```python
...
**(a) The sweep — `run_scan()`** ([scan360.py:269](scan360.py#L269)). Stop, settle, shoot,
...
**(b) Back-projection — `back_project()`** ([scan360.py:117](scan360.py#L117)): same equation
as §3.3, returning points + grayscale colors from `ir_left.png`.

**(c) Measuring the real rotation — `estimate_yaw()`** ([scan360.py:81](scan360.py#L81)).
...
**(d) Deciding which angle to trust — `cumulative_angles()`** ([scan360.py:182](scan360.py#L182)).
...
Y) axis** by its cumulative angle. The rotation matrix ([scan360.py:138](scan360.py#L138)):
```python
def ry(a):  # rotate about Y (vertical)
    c, s = cos(a), sin(a)
    return [[c, 0, s], [0, 1, 0], [-s, 0, c]]
```
`build_from_session()` ([scan360.py:217](scan360.py#L217)) ties it together:
```python
...
**(f) Downsample — `voxel_downsample()`** ([scan360.py:145](scan360.py#L145)). After stacking,
...
**(g) Saving — `write_ply()`** ([scan360.py:154](scan360.py#L154)) writes a **binary
little-endian colored PLY** (xyz + rgb). This exact format is what `view3d.py` reads back.

**CLI / standalone use** (no robot needed):
```bash
python3 scan360.py captures/scan_<ts>            # rebuild, MEASURED angle (default)
python3 scan360.py captures/scan_<ts> --known    # trust the timed step instead
python3 scan360.py captures/scan_<ts> --angle 40 # force a fixed step angle
python3 scan360.py --calibrate --turned 470      # pulsed calibration helper
```
...
- **Reads** the same PLY format `scan360.write_ply` produces ([view3d.py:34](view3d.py#L34)).
...
- **Theory — ICP (Iterative Closest Point).** Instead of trusting a single rotation angle, ICP
  *geometrically* snaps overlapping clouds together: repeatedly match nearest points and solve
  ...
  starts close, then refine ([merge_clouds.py:90](merge_clouds.py#L90), [:99](merge_clouds.py#L99)):
  ```python
...
- **What:** saves a front + top-down PNG of a `cloud.ply` (no interactive window). Handy for
  sanity-checking a cloud over SSH. ([render_cloud.py](render_cloud.py))
...
|---|---|---|
| [rs_capture.py](rs_capture.py) | Pi | D405 capture → capture folder (the producer) |
| [capture.py](capture.py) | Pi | standalone ENTER-to-capture |
| [scan360.py](scan360.py) | Pi | 360 sweep + measured-angle merge (the heart) |
| [view3d.py](view3d.py) | Pi/laptop | orbit a `.ply` (numpy+cv2 viewer) |
| [drive.py](drive.py) | Pi | WASD teleop + R/T/Y/V scan workflow (the conductor) |
| [make_pointcloud.py](make_pointcloud.py) | laptop | one capture → `cloud.ply` (Open3D) |
| [merge_clouds.py](merge_clouds.py) | laptop | many captures → `merged.ply` via ICP |
| [render_cloud.py](render_cloud.py) | laptop | static front/top preview PNG |
| [clean_captures.py](clean_captures.py) | either | delete captures / generated clouds |
