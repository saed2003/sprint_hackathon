...
Usage:
  python3 pointcloud/clean_captures.py            # asks before deleting
  python3 pointcloud/clean_captures.py --yes      # delete without asking
  python3 pointcloud/clean_captures.py --clouds   # keep raw captures, only delete generated
...
# operate on the project root (one level up from pointcloud/)
...
def main():
    ...
    capture_dirs = sorted(d for d in glob.glob(os.path.join(ROOT, "captures", "*"))
                          if os.path.isdir(d))
    merged = [os.path.join(ROOT, f) for f in ("merged.ply", "merged_views.png")
    ...
    if clouds_only:
        ...
        targets = merged + [
            ...
            for p in glob.glob(os.path.join(ROOT, "captures", "**", pat), recursive=True)
        ]
