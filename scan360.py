...
Standalone uses (no robot needed — rebuild from shots already captured):
  python3 scan360.py captures/scan_20260531_1700              # rebuild, MEASURED angle
  python3 scan360.py captures/scan_20260531_1700 --known      # trust the timed step
...
bot — it pulses EXACTLY like a scan, not one long spin, so the timing actually transfers):
  python3 scan360.py --calibrate                 # 8 scan-like pulses; measure total deg
  python3 scan360.py --calibrate --turned 470    # auto-prints the SCAN_SEC_PER_DEG to set
"""
...
# `python3 scan360.py --calibrate`. Default 6.0/360 just matches the old 6 s/rev guess.
...
def main():
    ...
    if not args:
        sys.exit("Usage:\n"
                 "  python3 scan360.py <session_dir>            # rebuild, measured angle\n"
                 "  python3 scan360.py <session_dir> --known    # trust the timed step\n"
                 "  python3 scan360.py <session_dir> --angle 40 # force a fixed step angle\n"
                 "  python3 scan360.py <session_dir> --dir -1   # flip rotation sign\n"
                 "  python3 scan360.py --calibrate [--shots N] [--speed S] [--turned DEG]")
