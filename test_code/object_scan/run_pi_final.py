...
This is the version to run in the live demo. It captures a fresh orbit like run_pi.py, but
at "build" time it GUARANTEES a clean car on screen: it loads a known-good, laptop-built car
cloud (committed next to this file as presentation_car_a.ply / _b.ply), converts it to the
simple binary .ply the Pi viewer reads, writes it as <session>/object_final.ply, and opens it
in the teammate's on-Pi point-cloud viewer (src/pointcloud/view3d.py).
...
fall back to the safe car if it fails / comes out empty.
    ...
    python3 run_pi_final.py --shots 36 --degrees 360 --radius 0.35   # capture knobs (-> run_pi)
...
def main():
    ...
    if build_only:
        ...
        print(f"=== run_pi_final: capture orbit {cfg['name']} (shots={s}, R={r*100:.0f}cm, {degrees:.0f}°) ===")
