...
```bash
cd ~/sprint_hackathon
python3 pointcloud/scan360.py --calibrate          # does 8 scan-like pulses; mark heading, watch it
```

Measure the **total degrees it turned**, then let it do the math for you:

```bash
python3 pointcloud/scan360.py --calibrate --turned 470   # prints the exact SCAN_SEC_PER_DEG to set
```
...
Edit `SCAN_SEC_PER_DEG` near the top of [scan360.py](../pointcloud/scan360.py), then re-run `--calibrate`
...
```bash
python3 pointcloud/scan360.py captures/scan_20260531_1700              # measured angle (default)
python3 pointcloud/scan360.py captures/scan_20260531_1700 --known      # trust the timed step instead
...
```bash
.venv/bin/python pointcloud/merge_clouds.py --angle 40 captures/scan_20260531_1700/shot_*/
```
...
.venv/bin/python pointcloud/make_pointcloud.py --all

# C) only the ones you name
.venv/bin/python pointcloud/make_pointcloud.py captures/20260531_120101 captures/20260531_120130
```
...
.venv/bin/python pointcloud/merge_clouds.py --angle 20

# or merge specific ones, in order
.venv/bin/python pointcloud/merge_clouds.py --angle 20 captures/A captures/B captures/C
```
...
python3 pointcloud/clean_captures.py

# same, without the confirmation prompt
python3 pointcloud/clean_captures.py --yes
...
python3 pointcloud/clean_captures.py --clouds
```
...
python3 pointcloud/scan360.py --calibrate --turned <deg>  # pulsed calibrate -> set SCAN_SEC_PER_DEG
python3 wasd/drive.py                               # drive; R = 360 capture, T = build, Y = view
                                               #         V = single capture
# result per spot: captures/scan_<ts>/merged_360.ply   (view with: python3 pointcloud/view3d.py <ply>)
...
# ── start over ──
python3 pointcloud/clean_captures.py                  # delete captures + merged output
```
